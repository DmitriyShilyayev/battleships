...
// Обработчик событий, выстрел игроком по вражескому полю
function hitHandler() {
    if (!isEnemyTurn) {
        let cell = $(this);
...
// Функция отвечающая за выстрел. Как игрока, так и противника
...
// Функция, выполняющая ход противника
function enemyTurn() {
    if (isEnemyTurn) {
        row = random(0, 9);
        column = random(0, 9);
...
// Функция, проверяющая, был ли корабль убит, и реагирующая на это
...
// Функция, рекурсией проверяющая, был корабль ранен или убит
...
// Пометить в массиве ячейки, где корабль уничтожен, сделать смежные с ним клетки неактивными, используя рекурсию
...
// Отобразить визуально на поле результаты выстрелов
...
// Уменьшить счётчик оставшихся кораблей противника при их уничтожении
...
// После уничтожения корабля, проверить, был ли это последний, объявить победу или проигрыш игрока
...
// Инициализировать обработчик кликов по полю противника
